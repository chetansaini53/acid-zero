/*
 * Acid Zero - All-in-One Co-Processor firmware (CC1101 + IR, ONE ESP32)
 * -----------------------------------------------------------------------
 * Merges firmware/esp32-cc1101 + firmware/esp32-ir onto a single 38-pin
 * ESP32-D0WD-V3 (NodeMCU-32S) board, so both radios travel as one physical
 * module instead of two separate boards. Every pin and every serial command
 * below is UNCHANGED from the two original sketches - this is a straight
 * merge, not a rewrite, so both existing Pi-side clients (acid_subghz.py,
 * acid_ir.py) keep working with zero changes: acid_subghz.py finds this
 * board via PING->PONG, acid_ir.py finds it via IR_INFO (a different probe
 * command, not PING), so there is no detection conflict on the shared port.
 * See ../../docs/pinout-esp32-allinone.svg for the full pin reference.
 *
 * GPS is NOT on this board - it wires directly to the Pi's own GPIO14/15
 * UART (see ../../launcher/acid_gps.py), not through this co-processor.
 *
 * Wiring (unchanged from the original sketches):
 *   CC1101   GND->GND  VCC->3V3  GDO0->D4  CSN->D5  SCK->D18  MOSI->D19  MISO->D23  GDO2->NC
 *   IR TX    LED driver S/DAT -> D13   VCC->3.3V  GND->GND
 *   IR RX    TSOP/VS1838 OUT  -> D14   VCC->3.3V  GND->GND
 *
 * Libraries: SmartRC-CC1101-Driver-Lib (ELECHOUSE) + IRremoteESP8266 >= 2.8.6
 *   arduino-cli lib install "SmartRC-CC1101-Driver-Lib" "IRremoteESP8266"
 *   Board (FQBN): esp32:esp32:esp32
 *
 * Serial protocol (one command per line, 115200 baud) - union of both:
 *   PING PONG VER INFO FREQ RSSI SCAN ANALYZE WATCH MOD SET_CONFIG CLASSIFY
 *   CAPTURE LOAD REPLAY                              (CC1101, unchanged)
 *   IR_INFO IR_RX IR_TX_RAW                          (IR, unchanged)
 *
 * Educational / authorized-lab use only. REPLAY / IR_TX_RAW transmit RF/IR -
 * use ONLY on your own devices / authorized test gear, on a frequency/power
 * that is legal in your region. See ../../ETHICS.md.
 *
 * NOT YET FLASHED/VERIFIED ON REAL HARDWARE - this is a straight source merge
 * of two independently-working sketches. Before trusting it: flash, then
 * repeat the verification steps from ../README.md (VER for CC1101, IR_INFO
 * for IR) to confirm both halves still work sharing one chip.
 */
...
// ============================================================================
// ---- CC1101 (Sub-GHz) - pins, state, helpers - unchanged from esp32-cc1101 ----
// ============================================================================
...
// ============================================================================
// ---- IR - pins, state, helpers - unchanged from esp32-ir ----
// ============================================================================
...
// ============================================================================
// ---- unified dispatch - union of both command sets, PING answers PONG so
// ---- acid_subghz.py's detection is unchanged; acid_ir.py detects via
// ---- IR_INFO (not PING), so both clients find this one board with no
...
static void handle(String cmd) {
  ...
  if (up == "PING") {
    ...
    Serial.println("ERR unknown. cmds: PING VER INFO FREQ RSSI SCAN ANALYZE WATCH CAPTURE REPLAY LOAD MOD SET_CONFIG CLASSIFY IR_INFO IR_RX IR_TX_RAW");
  }
}

void setup() {
  Serial.setRxBufferSize(8192);   // big IR_TX_RAW / LOAD lines (full raw frames from the Pi)
  Serial.begin(115200);
  delay(300);
  Serial.println();
  Serial.println("=== Acid Zero All-in-One co-processor (CC1101 + IR, one ESP32) ===");
  ccApplyPins();
  ccBringUp();
  ...
  g_irsend.begin();
  g_irrecv.enableIRIn();
  Serial.println("IR ready: RX=GPIO14 TX=GPIO13 (IRremoteESP8266).");
  Serial.println("cmds: PING VER INFO FREQ RSSI SCAN ANALYZE WATCH CAPTURE REPLAY LOAD | MOD <prof> SET_CONFIG --k v CLASSIFY | IR_INFO IR_RX IR_TX_RAW");
}
